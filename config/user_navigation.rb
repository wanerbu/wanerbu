...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    #                 be rendered (e.g. <tt>:if => Proc.new { current_user.user? }</tt>). The
    #                 proc should evaluate to a true or false value and is evaluated in the context of the view.
    #           :unless - Specifies a proc to call to determine if the item should not
    #                     be rendered (e.g. <tt>:unless => Proc.new { current_user.user? }</tt>). The
    #                     proc should evaluate to a true or false value and is evaluated in the context of the view.
    ...
    # 如果该用户没有经创建场馆，场馆管理菜不显示
    primary.item :gyms, I18n.t('user.nav.main_menu.common.management', model: Gym.model_name.human),
      user_master_gyms_path, {if: Proc.new {current_user.gym},:class => 'nav-header'} do |sub_nav|
...
#场地一览
        sub_nav.item :courts_list, I18n.t('user.nav.main_menu.common.list', model: Court.model_name.human), user_master_court_reports_path
...
#场次一览
    ...
    # primary.item :key_3, 'user', url, :class => 'special', :if => Proc.new { current_user.user? }
